...
type EncodedValves = Map.Map String Int
type Visited = [Int]
type Path = [String]
...
dp current remainingTime visited path currentFlow outcomes graph flow encodedValves =
    ...
    ) newOutcomes (Map.toList encodedValves)
...
main = do
    ...
    let encodedValves = Map.fromList $ zip (Map.keys flow) [0..]
    let initialVisited = replicate (Map.size encodedValves) 0
    let initialPath = []
