...
data AnimFrame
    ...
    | CheatFrame Point Point Int  -- Start, end, time saved
...
main = do
    ...
    case bfsWithDistances width height walls start end of
        ...
        Just (path, distMap) -> do
            let cheats = findCheats distMap path 2 1
                frames = buildFrames path cheats
                total = length frames
            bracket_ hideCursor showCursor $ do
                clearScreen
                mapM_ (renderFrame width height walls path) (zip [0 ..] frames)
            setCursorPosition (height + 10) 0
            putStrLn "Race Condition animation complete!"

loadMap :: IO String
loadMap = do
    let path = "test/day20 (example).csv"
...
buildFrames :: Path -> [(Point, Point, Int)] -> [AnimFrame]
buildFrames path cheats =
    -- First show the path being traced
    [PathFrame i pos | (i, pos) <- zip [0..] path] ++
    -- Then show some example cheats
    [CheatFrame start end saved | (start, end, saved) <- take 20 cheats]
...
renderFrame width height walls path (idx, frame) = do
    ...
    case frame of
        PathFrame step currentPos -> do
            putStrLn $ "Tracing main path: Step " ++ show step ++ " / " ++ show (length path - 1)
        ...
        CheatFrame cheatStart cheatEnd timeSaved -> do
            putStrLn $ "Showing cheat shortcut"
