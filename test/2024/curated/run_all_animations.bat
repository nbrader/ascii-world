@echo off
cd /d "%~dp0..\.."
echo ========================================
echo   Advent of Code 2024 - Animations
echo ========================================
echo.
echo Available animations:
echo   1. Day 6:  Guard Gallivant (Part 1 path, Part 2 loop cues)
echo   2. Day 10: Hoof It (Parts 1+2 shared BFS wave)
echo   3. Day 16: Reindeer Maze (Part 1 min-cost, Part 2 optimal tiles)
echo   4. Day 18: RAM Run (Part 1 path length, Part 2 blocker)
echo   5. Day 20: Race Condition (Part 1 short cheats, Part 2 long cheats)
echo.
echo   0. Exit
echo.
set /p choice="Enter your choice (0-5): "

if "%choice%"=="1" goto day06
if "%choice%"=="2" goto day10
if "%choice%"=="3" goto day16
if "%choice%"=="4" goto day18
if "%choice%"=="5" goto day20
if "%choice%"=="0" goto end
echo Invalid choice!
pause
goto menu

:day06
echo.
echo Running Day 6: Guard Gallivant (Part 1 path + Part 2 loop cues)...
echo.
stack --resolver lts-21.22 runghc --package containers-0.6.7 --package ansi-terminal-0.11.5 test/2024/day06_animated.hs
pause
goto menu

:day10
echo.
echo Running Day 10: Hoof It (Parts 1 + 2 shared BFS wave)...
echo.
stack --resolver lts-21.22 runghc --package containers-0.6.7 --package ansi-terminal-0.11.5 --package array test/2024/day10_animated.hs
pause
goto menu

:day16
echo.
echo Running Day 16: Reindeer Maze (Part 1 min-cost + Part 2 optimal tiles)...
echo.
stack --resolver lts-21.22 runghc --package containers-0.6.7 --package ansi-terminal-0.11.5 test/2024/day16_animated.hs
pause
goto menu

:day18
echo.
echo Running Day 18: RAM Run (Part 1 path + Part 2 blocking byte)...
echo.
stack --resolver lts-21.22 runghc --package containers-0.6.7 --package ansi-terminal-0.11.5 test/2024/day18_animated.hs
pause
goto menu

:day20
echo.
echo Running Day 20: Race Condition (Part 1 short cheats + Part 2 long cheats)...
echo.
stack --resolver lts-21.22 runghc --package containers-0.6.7 --package ansi-terminal-0.11.5 test/2024/day20_animated.hs
pause
goto menu

:menu
cls
cd /d "%~dp0..\.."
echo ========================================
echo   Advent of Code 2024 - Animations
echo ========================================
echo.
echo Available animations:
echo   1. Day 6:  Guard Gallivant (Part 1 path, Part 2 loop cues)
echo   2. Day 10: Hoof It (Parts 1+2 shared BFS wave)
echo   3. Day 16: Reindeer Maze (Part 1 min-cost, Part 2 optimal tiles)
echo   4. Day 18: RAM Run (Part 1 path length, Part 2 blocker)
echo   5. Day 20: Race Condition (Part 1 short cheats, Part 2 long cheats)
echo.
echo   0. Exit
echo.
set /p choice="Enter your choice (0-5): "

if "%choice%"=="1" goto day06
if "%choice%"=="2" goto day10
if "%choice%"=="3" goto day16
if "%choice%"=="4" goto day18
if "%choice%"=="5" goto day20
if "%choice%"=="0" goto end
echo Invalid choice!
pause
goto menu

:end
echo.
echo Thanks for watching the animations!
echo.
